...
export default {
  ...
  methods: {
    ...
    handleDrop(event) {
      ...
      if (dataTransfer) {
        ...
        if (node) {
          ...
          // console.log("handleDrop", cloneDeep(node), cloneDeep(list));
    ...
    getEquipTroops() {
      let equips = 0,
        troops = 0;
      // let loopFn = function (arr) {
      //   arr.forEach((item) => {
      //     let { children, weapon, army } = item;
      ...
      // };
      // loopFn(arr);
      const { childrenEquips, childrenTroops } = this.nodeData;
      if (childrenEquips) {
        childrenEquips.forEach((i) => (equips += i.num));
      }
      if (childrenTroops) {
        childrenTroops.forEach((i) => (troops += i.num));
      }
